...
function formatINR(n) {
  const x = Number(n);
  if (!Number.isFinite(x)) return "â‚¹0";
  try {
...
function getFundSources(state) {
  ...
  for (const inv of state.investments || []) {
    sources.push({
      id: inv.id,
      kind: "investment",
      label: inv.name || investmentTypeLabels[inv.type] || "Investment",
    ...
    investments = (unknown.investments || [])
      ...
      .map((inv) => {
        ...
        return {
          ...
          frequency: inv.isRecurring ? String(inv.frequency || "monthly") : "",
...
const GOAL_THEMES = [
  { icon: "ðŸ›¡", iconBg: "bg-amber-500/15", bar: "bg-amber-400" },
  { icon: "ðŸ ", iconBg: "bg-emerald-500/15", bar: "bg-emerald-400" },
  { icon: "â˜€", iconBg: "bg-sky-500/15", bar: "bg-sky-400" },
  { icon: "âœˆ", iconBg: "bg-orange-500/15", bar: "bg-orange-400" },
  { icon: "ðŸŽ“", iconBg: "bg-violet-500/15", bar: "bg-violet-400" },
  { icon: "ðŸ’°", iconBg: "bg-teal-500/15", bar: "bg-teal-400" },
];
...
function formatCompactINR(n) {
  ...
  if (x >= 10000000) return `â‚¹${(x / 10000000).toFixed(1)}Cr`;
  if (x >= 100000) return `â‚¹${(x / 100000).toFixed(1)}L`;
  if (x >= 1000) return `â‚¹${(x / 1000).toFixed(1)}K`;
  return formatINR(x);
}
  ...
  return (
    <div className={`${cls.card} p-5 transition hover:border-white/20`}>
      ...
      <p className="mt-1 text-sm text-slate-500">
        Target Â· {formatTargetDate(goal.targetDate)}
        {goal.targetAmount > 0 ? ` Â· ${formatCompactINR(goal.targetAmount)}` : ""}
      </p>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">Private Â· stored in your browser</div>
            </div>
          </div>
...
function DashboardScreen({ goals, summaries, metrics }) {
  return (
    <div className={`${cls.page} py-8`}>
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className={`${cls.card} p-5`}>
          ...
          <div className="mt-2 text-2xl font-semibold text-white">{formatINR(metrics.netWorth)}</div>
          <div className="mt-1 text-xs text-slate-500">Bank balances + investments</div>
        </div>
        <div className={`${cls.card} p-5`}>
        ...
        <div className={`${cls.card} p-5`}>
          <div className={cls.label}>Monthly investable surplus</div>
          <div className={`mt-2 text-2xl font-semibold ${metrics.investableSurplus >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
            {formatINR(metrics.investableSurplus)}
          </div>
          <div className="mt-1 text-xs text-slate-500">Income âˆ’ EMIs âˆ’ expenses</div>
        </div>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 text-xs text-slate-500">
                      {formatTargetDate(g.targetDate)} Â· {formatCompactINR(g.targetAmount)}
                    </div>
                  </button>
                );
              })}
              ...
              <div>
                ...
                {isEmergency ? (
                  <div className={`${cls.input} flex items-center justify-between bg-white/[0.03] text-slate-300`}>
                    ...
                    <span className="text-xs text-amber-300">Auto: 6Ã— monthly expenses</span>
                  </div>
                ) : (
                  <input
                    type="number"
...
function AllocationEditor({ goals, amount, allocations, setAllocations }) {
    ...
    <div className={`${cls.card} mt-6 p-4`}>
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-slate-500">Total must be {"â‰¤"} 100%</div>
        </div>
...
function FinancialDataScreen({
  ...
  const emergencyNote = `Emergency fund target auto-updates to ${EMERGENCY_FUND_MONTHS}Ã— your monthly expenses (${formatINR(emergencyFundTarget(monthlyExpenses))}).`;
            ...
            {commitments.map((c) => (
              <div key={c.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <div>
                    <label className={cls.label}>Loan / lender</label>
                    <input
                      className={cls.input}
                      value={c.name}
                      onChange={(e) => onUpdateCommitment(c.id, { name: e.target.value })}
                      placeholder="Home loan â€” SBI"
                    />
                  </div>
                  <div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">MFs, stocks, FDs, RDs, policies â€” allocate to goals</p>
            </div>
          </div>
            ...
            <table className="min-w-[760px] w-full">
              ...
              <tbody>
                {investments.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id}>
                      <td className={cls.td}>{inv.name || "â€”"}</td>
                      <td className={cls.td}>{investmentTypeLabels[inv.type]}</td>
                      <td className={cls.td}>{inv.bankName || "â€”"}</td>
                      <td className={cls.td}>{formatINR(inv.outstandingAmount)}</td>
                      <td className={cls.td}>
                        {inv.isRecurring ? `${inv.frequency || "monthly"}${inv.maturityDate ? ` Â· mat. ${inv.maturityDate}` : ""}` : "â€”"}
                      </td>
                      <td className={`${cls.td} max-w-xs truncate text-slate-500`}>{allocText}</td>
                      <td className={cls.td}>
                        <div className="flex gap-2">
...
function InvestmentModal({ open, mode, investment, goalList, onClose, onSave }) {
  ...
  const amountForAlloc = safeNumber(outstandingAmount);
  const unallocated = Math.max(0, amountForAlloc - computeHoldingAllocatedTotal({ amount: amountForAlloc, allocations }));
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-3">
            ...
            <button
              className={cls.btnPrimary}
              onClick={() => {
                if (!String(name || "").trim()) return alert("Please enter a name or label.");
                ...
                if (!zeroAmountType && amt <= 0) return alert("Outstanding amount must be greater than 0.");
                const allocClean = normalizeAllocations(allocations);
                const sumPct = Object.values(allocClean).reduce((a, v) => a + safeNumber(v), 0);
                if (sumPct > 100.001) return alert("Total allocation must be 100% or less.");
                onSave({
    ...
    >
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <input className={cls.input} value={name} onChange={(e) => setName(e.target.value)} placeholder="SBI Bluechip / LIC policy" />
        </div>
        <div className="sm:col-span-2">
          <label className={cls.label}>Bank</label>
          <BankSelect value={bankName} onChange={setBankName} />
        ...
        {isRecurring ? (
          <>
            ...
            <div>
              <label className={cls.label}>Maturity amount (optional)</label>
              <input
                type="number"
                min="0"
                className={cls.input}
            ...
            <div className="sm:col-span-2">
              <label className={cls.label}>Maturity date (optional)</label>
              <input type="date" className={cls.input} value={maturityDate} onChange={(e) => setMaturityDate(e.target.value)} />
            </div>
          </>
        ) : null}
      </div>
      <AllocationEditor goals={goalList || []} amount={amountForAlloc} allocations={allocations} setAllocations={setAllocations} />
    </Modal>
  );
}
...
function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {activeTab === "dashboard" ? (
        <DashboardScreen goals={orderedGoals} summaries={dashboardSummaries} metrics={dashboardMetrics} />
      ) : null}
        ...
        <FinancialDataScreen
          ...
          onAddCommitment={() =>
            setStateAndBump((prev) => ({
              ...
              commitments: [
                ...prev.commitments,
                { id: generateId(), name: "", outstandingAmount: 0, monthlyEmi: 0 },
              ],
            }))
          }
