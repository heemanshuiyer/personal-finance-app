...
function investmentDisplayName(inv) {
...
function commitmentDisplayName(c) {
  return (c && String(c.description || "").trim()) || "Untitled commitment";
}

/** Cumulative EMIs from today through isoDate, capped per commitment by remainingTenure. */
function computeCommitmentEmisThroughDate(commitments, isoDate, selectedCommitmentIds) {
  const today = todayLocalISODate();
  const monthsToDate = Math.max(0, diffMonths(today, isoDate));
  let total = 0;
  for (const c of commitments || []) {
    if (selectedCommitmentIds && !selectedCommitmentIds.has(c.id)) continue;
    const tenure = Math.max(0, Math.floor(safeNumber(c.remainingTenure)));
    if (tenure <= 0) continue;
    const months = Math.min(monthsToDate, tenure);
    total += safeNumber(c.monthlyEmi) * months;
  }
  return total;
}

function computeRequiredCorpusAtDate(state, goals, isoDate, filters) {
  const d = parseISODate(isoDate);
  if (!d) return 0;
  const selectedGoalIds = filters?.goalIds || null;
  const selectedCommitmentIds = filters?.commitmentIds || null;
  ...
  let emergencyDue = 0;
  if (efGoal && (!selectedGoalIds || selectedGoalIds.has(efGoal.id))) {
    emergencyDue =
      parseISODate(efGoal.targetDate) && monthIndex(parseISODate(efGoal.targetDate)) <= monthIndex(d)
        ? safeNumber(efGoal.targetAmount)
        : emergencyNeed;
  }

  const commitmentEmis = computeCommitmentEmisThroughDate(
    state.commitments || [],
    isoDate,
    selectedCommitmentIds
  );
  return emergencyDue + commitmentEmis + goalsDue;
}

function computeAvailableCorpusAtDate(state, isoDate, filters) {
  const selectedBankIds = filters?.bankIds || null;
  const selectedInvestmentIds = filters?.investmentIds || null;
  const today = todayLocalISODate();
  ...
  let total = 0;
  for (const b of state.bankAccounts || []) {
    if (selectedBankIds && !selectedBankIds.has(b.id)) continue;
    total += safeNumber(b.balance);
  }
  for (const inv of state.investments || []) {
    if (selectedInvestmentIds && !selectedInvestmentIds.has(inv.id)) continue;
    total += safeNumber(inv.outstandingAmount);
  }

  if (!target || !todayD) return total;
  if (monthIndex(target) <= monthIndex(todayD)) return total;
  ...
  for (const inv of state.investments || []) {
    if (selectedInvestmentIds && !selectedInvestmentIds.has(inv.id)) continue;
    if (inv.recurringAllocationMode !== "maturity") continue;
    const matAmt = safeNumber(inv.maturityAmount);
    const matDate = inv.maturityDate;
...
function computeOutlookChartData(state, goals, filters) {
  const today = todayLocalISODate();
  let endISO = today;
  for (const g of goals || []) {
  ...
  for (const c of state.commitments || []) {
    const tenure = Math.max(0, Math.floor(safeNumber(c.remainingTenure)));
    if (tenure > 0) {
      const end = addMonthsISO(today, tenure);
  ...
  return quarters.map((iso) => {
    const required = computeRequiredCorpusAtDate(state, goals, iso, filters?.required);
    const available = computeAvailableCorpusAtDate(state, iso, filters?.projected);
    return {
      iso,
  ...
  if (version === 1) {
    ...
    commitments = (unknown.commitments || [])
      ...
      .map((c) => ({
        ...
        remainingTenure: Math.max(0, Math.floor(safeNumber(c.remainingTenure))),
      }));
  } else {
    return null;
  }
...
function HierarchyMultiSelect({ label, groups, selectedIds, onChange }) {
  ...
  const summary =
    selectedCount === 0
      ? "None selected"
      : selectedCount === allChildIds.length
        ? "All selected"
  ...
  return (
    <div className="relative min-w-[220px] flex-1" ref={rootRef}>
      <div className={cls.label}>{label}</div>
      <button
        type="button"
        className={`${cls.input} mt-2 flex items-center justify-between text-left`}
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open ? (
        <div className={`${cls.card} absolute left-0 right-0 z-40 mt-2 max-h-72 overflow-y-auto p-2 shadow-2xl`}>
          {groups.map((group) => {
            const childIds = group.children.map((c) => c.id);
            const selectedInGroup = childIds.filter((id) => selectedIds.has(id)).length;
            const allOn = childIds.length > 0 && selectedInGroup === childIds.length;
              ...
              <div key={group.id} className="mb-2 last:mb-0">
                ...
                <div className="ml-5 space-y-0.5">
                  {group.children.length === 0 ? (
                    <div className="px-2 py-1 text-xs text-slate-500">None yet</div>
                  ) : (
                    group.children.map((child) => (
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => onChange(toggleIdInSet(selectedIds, child.id, e.target.checked))}
                        />
                        <span className="truncate text-sm text-slate-300">{child.label}</span>
                      </label>
                    ))
                  )}
                </div>
...
function OutlookScreen({ state, goals }) {
  const bankIds = useMemo(() => (state.bankAccounts || []).map((b) => b.id), [state.bankAccounts]);
  const investmentIds = useMemo(() => (state.investments || []).map((inv) => inv.id), [state.investments]);
  const commitmentIds = useMemo(() => (state.commitments || []).map((c) => c.id), [state.commitments]);
  const goalIds = useMemo(() => (goals || []).map((g) => g.id), [goals]);

  const [selectedBankIds, setSelectedBankIds] = useState(() => new Set(bankIds));
  const [selectedInvestmentIds, setSelectedInvestmentIds] = useState(() => new Set(investmentIds));
  const [selectedCommitmentIds, setSelectedCommitmentIds] = useState(() => new Set(commitmentIds));
  const [selectedGoalIds, setSelectedGoalIds] = useState(() => new Set(goalIds));

  const prevBankIdsRef = useRef(bankIds);
  const prevInvestmentIdsRef = useRef(investmentIds);
  const prevCommitmentIdsRef = useRef(commitmentIds);
  const prevGoalIdsRef = useRef(goalIds);

  useEffect(() => {
    const prevList = prevBankIdsRef.current;
    setSelectedBankIds((prev) => {
      const wasFullySelected = prev.size === 0 || prevList.every((id) => prev.has(id));
      const next = new Set(bankIds.filter((id) => prev.has(id)));
      for (const id of bankIds) {
        if (!prevList.includes(id) && wasFullySelected) next.add(id);
      }
      return next;
    });
    prevBankIdsRef.current = bankIds;
  }, [idsKey(bankIds)]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    const prevList = prevInvestmentIdsRef.current;
    setSelectedInvestmentIds((prev) => {
      const wasFullySelected = prev.size === 0 || prevList.every((id) => prev.has(id));
      const next = new Set(investmentIds.filter((id) => prev.has(id)));
      for (const id of investmentIds) {
        if (!prevList.includes(id) && wasFullySelected) next.add(id);
      }
      return next;
    });
    prevInvestmentIdsRef.current = investmentIds;
  }, [idsKey(investmentIds)]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    const prevList = prevCommitmentIdsRef.current;
    setSelectedCommitmentIds((prev) => {
      const wasFullySelected = prev.size === 0 || prevList.every((id) => prev.has(id));
      const next = new Set(commitmentIds.filter((id) => prev.has(id)));
      for (const id of commitmentIds) {
        if (!prevList.includes(id) && wasFullySelected) next.add(id);
      }
      return next;
    });
    prevCommitmentIdsRef.current = commitmentIds;
  }, [idsKey(commitmentIds)]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    const prevList = prevGoalIdsRef.current;
    setSelectedGoalIds((prev) => {
      const wasFullySelected = prev.size === 0 || prevList.every((id) => prev.has(id));
      const next = new Set(goalIds.filter((id) => prev.has(id)));
      for (const id of goalIds) {
        if (!prevList.includes(id) && wasFullySelected) next.add(id);
      }
      return next;
    });
    prevGoalIdsRef.current = goalIds;
  }, [idsKey(goalIds)]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    () => [
      {
        id: "banks",
        label: "Bank Accounts",
        children: (state.bankAccounts || []).map((b) => ({
          id: b.id,
          label: b.bankName || "Bank account",
        })),
      },
      {
        ...
        children: (state.investments || []).map((inv) => ({
          id: inv.id,
          label: investmentDisplayName(inv),
        })),
      },
    ],
    [state.bankAccounts, state.investments]
  );
  ...
  const requiredGroups = useMemo(
    () => [
      {
        id: "commitments",
        label: "Commitments",
        children: (state.commitments || []).map((c) => ({
          id: c.id,
          label: commitmentDisplayName(c),
        })),
      },
      {
        id: "goals",
    ...
    [state.commitments, goals]
  );

  const projectedSelected = useMemo(() => {
    const next = new Set(selectedBankIds);
    for (const id of selectedInvestmentIds) next.add(id);
    return next;
  }, [selectedBankIds, selectedInvestmentIds]);

  const setProjectedSelected = (nextSet) => {
    const banks = new Set();
    const invs = new Set();
    for (const id of nextSet) {
      if (bankIds.includes(id)) banks.add(id);
      if (investmentIds.includes(id)) invs.add(id);
    }
    setSelectedBankIds(banks);
    setSelectedInvestmentIds(invs);
  };

  const requiredSelected = useMemo(() => {
    const next = new Set(selectedCommitmentIds);
    for (const id of selectedGoalIds) next.add(id);
    return next;
  }, [selectedCommitmentIds, selectedGoalIds]);

  const setRequiredSelected = (nextSet) => {
    const commits = new Set();
    const gs = new Set();
    for (const id of nextSet) {
      if (commitmentIds.includes(id)) commits.add(id);
      if (goalIds.includes(id)) gs.add(id);
    }
    setSelectedCommitmentIds(commits);
    setSelectedGoalIds(gs);
  };

  const chartData = useMemo(
    () =>
      computeOutlookChartData(state, goals, {
        projected: { bankIds: selectedBankIds, investmentIds: selectedInvestmentIds },
        required: { commitmentIds: selectedCommitmentIds, goalIds: selectedGoalIds },
      }),
    [state, goals, selectedBankIds, selectedInvestmentIds, selectedCommitmentIds, selectedGoalIds]
  );

  const insights = useMemo(() => computeOutlookInsights(chartData, goals), [chartData, goals]);
    ...
    <div className={`${cls.page} py-8`}>
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row">
        <HierarchyMultiSelect
          ...
          selectedIds={projectedSelected}
          onChange={setProjectedSelected}
        />
        <HierarchyMultiSelect
          label="Total required corpus"
          groups={requiredGroups}
          selectedIds={requiredSelected}
          onChange={setRequiredSelected}
        />
      </div>
      ...
      <div className={`${cls.card} mt-4 p-4 text-sm leading-relaxed text-slate-400`}>
        ...
        overtakes red, you are ahead. Commitment EMIs are included in required corpus through each loan&apos;s remaining
        tenure.
      </div>
    </div>
  );
            ...
            {commitments.map((c) => (
              <div key={c.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
                  ...
                  <div>
                    <label className={cls.label}>Remaining tenure</label>
                    <input
                      type="number"
                      min="0"
                      step="1"
                      className={cls.input}
                      value={c.remainingTenure || ""}
                      onChange={(e) =>
                        onUpdateCommitment(c.id, {
                          remainingTenure: Math.max(0, Math.floor(parseNumberInput(e.target.value))),
                        })
                      }
                      placeholder="Months"
                    />
            ...
            <table className="min-w-[880px] w-full">
              <thead>
                <tr>
                  ...
                  <th className={cls.th}>Recurring</th>
                  <th className={cls.th}>Recurring amount</th>
                  <th className={cls.th}>Maturity amount</th>
                  <th className={cls.th}>Goals</th>
                  <th className={cls.th}></th>
                </tr>
              </thead>
                ...
                {investments.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td className={cls.td}>{maturityDisplay}</td>
                      <td className={`${cls.td} max-w-xs truncate text-slate-500`}>{allocText}</td>
                      <td className={cls.td}>
                        <div className="flex gap-2">
    ...
    <div className="min-h-screen">
      ...
      {view === "landing" ? (
        ...
        <>
            ...
            <FinancialDataScreen
              ...
              onAddCommitment={() =>
                setStateAndBump((prev) => ({
                  ...
                  commitments: [
                    ...prev.commitments,
                    { id: generateId(), description: "", bankName: BANK_NAMES[0], outstandingAmount: 0, monthlyEmi: 0, remainingTenure: 0 },
                  ],
                }))
              }
          ...
          {activeTab === "outlook" ? (
            <OutlookScreen state={state} goals={orderedGoals} />
          ) : null}
